...
public class CertainBookStore implements BookStore, StockManager {
	...
	public synchronized List<Book> getTopRatedBooks(int numBooks) throws BookStoreException {
		...
		List<BookStoreBook> sortedTopRatedBooks = bookMap.values().stream()
				.sorted(Comparator.comparing(BookStoreBook::getAverageRating, Comparator.reverseOrder()))
				.collect(Collectors.toList());

		//if number of books asked is bigger than all available books,
		//we need to retrieve them all, otherwise we retrieve only the numBook first books
		if (numBooks <= sortedTopRatedBooks.size()) {
			sortedTopRatedBooks = sortedTopRatedBooks.stream().limit(numBooks).collect(Collectors.toList());
		}

		return sortedTopRatedBooks.stream().map(BookStoreBook::immutableBook)
				.collect(Collectors.toList());
	}
